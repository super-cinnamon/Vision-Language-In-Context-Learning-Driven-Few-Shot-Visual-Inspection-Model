...
if __name__ == "__main__":
    ...
    ksdd2_prompt = json.load(open('jsons/eval/ksdd2/prompt_single.json'))
    for idx in range(len(weight_path)):
        ...
        for weight in weights:
            ...
            os.makedirs(os.path.join('result/ksdd2/mono', timestamp),  exist_ok=True)
            # Setup
            with open(os.path.join('result/ksdd2/mono', timestamp, 'output_full_text.csv'), 'w') as f: pass
            ...
            for data_path in data_paths:
                input_text = ksdd2_prompt[data_path]
                for i, text in enumerate(predictor.predict(image=[data_path], prompt=input_text, temperature=1e-5)):
                    ...
                    if tmp_mode != data_path.split('/')[-2]:
                        with open(os.path.join('result/ksdd2/mono', timestamp, 'output_full_text.csv'), 'a') as f:
                            writer = csv.writer(f)
