...
if __name__ == "__main__":
    ...
    visa_pairs = json.load(open('jsons/eval/visa/rices_one.json'))
    for idx in range(len(weight_path)):
        ...
        for weight in weights:
            ...
            os.makedirs(os.path.join('result/visa/rices', timestamp),  exist_ok=True)
            # Setup
            with open(os.path.join('result/visa/rices', timestamp, 'output_full_text.csv'), 'w') as f: pass
            ...
            for data_path in data_paths:
                ...
                for i, text in enumerate(predictor.predict(image=[context, data_path], prompt=input_text, temperature=1e-5)):
                    ...
                    if tmp_mode != data_path.split('/')[-2]:
                        with open(os.path.join('result/visa/rices', timestamp, 'output_full_text.csv'), 'a') as f:
                            writer = csv.writer(f)
