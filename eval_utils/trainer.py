...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                # inputs: {input_ids, labels, attention_mask, images}]
            ...
            # HACK: evaluate at the end of the epoch
            ...
            if (epoch+1) % args.save_epochs == 0:
                self.save_model(f'{args.output_dir}/{epoch+1}')
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        # if self.args.local_rank==0:
            # ASSISTANT: 以降をデコードする
            # print(inputs['labels'][0])
            # print(torch.where(inputs['labels'][0] == 319))
            # idx = torch.where(inputs['labels'][0] == 319)[0].item()
            # print('###########OUTPUTS###########')
            # only_answer = []
            # print(outputs.shape)
            # print(outputs['logits'][0].shape)
            # for output in outputs['logits'][0]:
            #     only_answer.append(torch.argmax(output).item() if torch.argmax(output).item() != -100 else 0)
            # only_answer = [torch.argmax(output).item() for output in outputs['logits'][0][(idx-1):]]
            # only_answer_label = [torch.argmax(output).item() for output in inputs['labels'][0][(idx-1):]]
            
            # print(outputs['logits'][0].shape)
            # print(len(only_answer))
            # print(only_answer)
            # print(self.tokenizer.decode(only_answer))
            # print('###########LABELS###########')
            # print(inputs['labels'][0])
            # print(self.tokenizer.decode(only_answer_label))
