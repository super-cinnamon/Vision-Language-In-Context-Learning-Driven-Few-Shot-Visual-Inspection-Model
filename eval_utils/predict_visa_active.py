...
if __name__ == "__main__":
    ...
    visa_prompt = json.load(open('jsons/eval/visa/prompt_each.json'))
    visa_pairs = json.load(open('jsons/eval/visa/nearest_first.json'))
    for idx in range(len(weight_path)):
        ...
        for weight in weights:
            ...
            os.makedirs(os.path.join('result/visa/active', timestamp),  exist_ok=True)
            # Setup
            with open(os.path.join('result/visa/active', timestamp, 'output_full_text.csv'), 'w') as f: pass
            ...
            for data_path in data_paths:
                context = visa_pairs[data_path]
                input_text = visa_prompt[context]
                for i, text in enumerate(predictor.predict(image=[context, data_path], prompt=input_text, temperature=1e-5)):
                    ...
                    if tmp_mode != data_path.split('/')[-2]:
                        with open(os.path.join('result/visa/active', timestamp, 'output_full_text.csv'), 'a') as f:
                            writer = csv.writer(f)
